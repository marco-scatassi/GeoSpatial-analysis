...
class AdjacencyMatrix:
    ...
    def __init__(self, adj_matrix=None, coordinates: gpd.geoseries.GeoSeries=[], kind="geometric", epsg=32610, mode="distance"):
        ...
        if self.kind == "geometric":
            ...
            for i in range(chunks):
                ...
                for j in range(chunks):
                    ...
                    sources_url = "sources="+";".join(str(k-100*i) for k in source_indexes)  
                    destinations_url = "destinations="+";".join(str(k-100*j+shift) for k in destination_indexes)
                    
                    # define the coordinates URL
                    coordinates_url = "".join(["".join(str(str([coordinates_list[i][0],coordinates_list[i][1]]))[1:-1].split())+";" for i in source_indexes+destination_indexes])[:-1]
...
class FacilityLocation:
    ...
    def plot(self):
        ...
        ax.scatter([c.x for c in self.coordinates.geometry], [c.y for c in self.coordinates.geometry], c = 'blue')
        ax.scatter([c[0].x for c in self.locations_coordinates], [c[0].y for c in self.locations_coordinates], c = 'red')
            ...
            for j, weight_name in enumerate(weight_names_and_index.keys()):
                for k in weight_names_and_index[weight_name]:
                    if self.facility_locations[k].algorithm == algorithm_name:
                        ...
                        axs[j,i].scatter([x for x in np.array(self.facility_locations[k].coordinates)[:,0]], \
                            [x for x in np.array(self.facility_locations[k].coordinates)[:,1]], c = 'blue')
                        axs[j,i].scatter([x for x in np.array(self.facility_locations[k].locations_coordinates)[:,0]], \
                            [x for x in np.array(self.facility_locations[k].locations_coordinates)[:,1]], c = 'red')
                        axs[j,i].text(0.05, 0.2, f"points: {self.facility_locations[k].n_of_demand_points} \ntime: {round(self.facility_locations[k].computation_time,3)} s\nsolution: {round(self.facility_locations[k].solution_value, 2)} "+unit, 
