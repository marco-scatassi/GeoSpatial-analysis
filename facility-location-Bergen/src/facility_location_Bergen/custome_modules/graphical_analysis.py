...
def facilities_on_map(fls, extra_text=None, fl_classes="not-provided", title_pad_l=50):
    ...
    for i in range(len(fls)):
        ...
        if "StochasticFacilityLocation" in str(type(fls[i])):
            ...
            mapping[(f"deterministic_{extra_text[i]}", fls[i].n_of_locations_to_choose, fl_class)] = fls[i]
...
def compute_CI(df_min, extra_text=""):
    index = []
    ...
    for k in df_min_keys:
        for col in df_min.columns[1:]:
            ...
            for _ in range(n_iterations):
                bootstrap_sample = resample(df_min[k][col], replace=True, n_samples=len(df_min[k]))
