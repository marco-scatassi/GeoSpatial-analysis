...
from retrieve_global_parameters import retrieve_average_graph_path, retrieve_catalog_path, retrieve_adj_matrix_path, retrieve_adj_mapping_path
...
def build_adj_matrix(time):
    ...
    if time == "all_day":
        sp_free_flow = dict(nx.all_pairs_dijkstra_path_length(average_graph, weight="weight2"))
...
def update_data_catalog(time, trigger):
    if trigger:
        ...
        with open(catalog_path, "r+") as f:
            ...
            result = re.search(fr"build_adjacency_matrix.{time}.adj_matrix.{time}:", contents)
            if result is None:
                    contents = "\n".join([contents, 
                                    "\n    ".join([f"build_adjacency_matrix.{time}.adj_matrix.{time}:",
        ...
        with open(catalog_path, "r+") as f:
            contents = f.read()
            adj_mapping_path = retrieve_adj_mapping_path(time)
            result = re.search(fr"build_adjacency_matrix.{time}.adj_mapping_matrix.{time}:", contents)
            if result is None:
                    contents = "\n".join([contents, 
                                    "\n    ".join([f"build_adjacency_matrix.{time}.adj_mapping_matrix.{time}:",
                                                f"type: pickle.PickleDataSet",
