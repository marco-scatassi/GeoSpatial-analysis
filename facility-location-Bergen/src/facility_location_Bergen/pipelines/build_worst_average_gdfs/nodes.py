...
from retrieve_global_parameters import (
    retrieve_gdf_worst_average_path,
    retrieve_gdf_path,
...
def take_average_worst_case_gdf_sub(gdf):
    gdf = gdf[gdf["currentFlow.traversability"] == "open"]

    gdf["hours_minutes_seconds"] = gdf.apply(
        lambda x: x["api_call_time"].time(), axis=1
    )
    ...
    gdf_worst_average = create_average_gdf_sub(gdf_worst)
...
# --------------------------------------- create the average gdf ---------------------------------------
def create_worst_average_gdfs(days):
    finished = False
    gdfs = []
    for day in days:
        try:
    ...
    gdf_average = take_average_worst_case_gdf_sub(gdf)
    gdf_morning_average = take_average_worst_case_gdf_sub(gdf_morning)
    gdf_midday_average = take_average_worst_case_gdf_sub(gdf_midday)
    gdf_afternoon_average = take_average_worst_case_gdf_sub(gdf_afternoon)
    ...
    for key in gdfs_average_merged.keys():
        # save gdf
        saving_path = retrieve_gdf_worst_average_path(key)
    ...
    if trigger:
        ...
        with open(catalog_path, "r+") as f:
            ...
            for key in ["all_day", "morning", "midday", "afternoon"]:
                result = re.search(rf"gdf_worst_average_{key}:", contents)
                gdf_path = retrieve_gdf_worst_average_path(key)
                if result is None:
                    contents = "\n".join(
                        [
                            contents,
                            "\n    ".join(
                                [
                                    f"build_worst_average_gdfs.{key}.gdf_worst_average_{key}:",
