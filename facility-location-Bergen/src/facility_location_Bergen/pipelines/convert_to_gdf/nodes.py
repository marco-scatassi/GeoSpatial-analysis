...
def from_json_to_gdf(date: dict, already_created: bool):
    ...
    if not already_created:
        ...
        first_date = datetime.strptime(
            f"{date['day']}T07:30:00.000+02:00", "%d_%m_%YT%H:%M:%S.%f%z"
        )
        last_date = datetime.strptime(
            f"{date['day']}T17:00:00.000+02:00", "%d_%m_%YT%H:%M:%S.%f%z"
        )
        # retrieve database and collections
        db, collection = retrieve_database_and_collections(
            db_name, date["day"], ["clean"]
        )
        ...
        df = pd.json_normalize(
            clean_collection.find(
                {"api_call_time": {"$gte": first_date, "$lte": last_date}}
            )
        )
        # process dataframe columns
        df["_id"] = df.apply(lambda x: str(x["_id"]), axis=1)
        df["geometry.multi_line"] = df.apply(
            lambda x: toMultiLineString(x["geometry.geometries"]), axis=1
        )
        df["geometry.extreme_point"] = df.apply(
            lambda x: toExtremePoints(x["geometry.multi_line"]), axis=1
        )
        df["api_call_time"] = df.apply(
            lambda x: x["api_call_time"].tz_localize("UTC"), axis=1
        )
        df["sourceUpdated"] = df.apply(
            lambda x: x["sourceUpdated"].tz_localize("UTC"), axis=1
        )
...
def get_length(data):
    ...
    for i in range(n):
        total_difference += abs(
            data["currentFlow.subSegments"][i]["length"] - sum(length_list[i])
        )
...
def new_rows(data, gdf):
    ...
    for i in range(n):
        ...
        new_rows[i]["geometry.extreme_point"] = [
            toExtremePoints(MultiLineString(geometry_list[i]))
        ]
        new_rows[i]["geometry_length"] = [length_list[i]]
        new_rows[i]["currentFlow.subSegments"] = np.nan
        new_rows[i]["currentFlow.traversability"] = data["currentFlow.subSegments"][i][
            "traversability"
        ]

        try:
            new_rows[i]["currentFlow.junctionTraversability"] = data[
                "currentFlow.subSegments"
            ][i]["junctionTraversability"]
        except:
            new_rows[i]["currentFlow.junctionTraversability"] = np.nan

        try:
            new_rows[i]["currentFlow.jamTendency"] = data["currentFlow.subSegments"][i][
                "jamTendency"
            ]
        except:
        ...
        try:
            new_rows[i]["currentFlow.confidence"] = data["currentFlow.subSegments"][i][
                "confidence"
            ]
        except:
        ...
        try:
            new_rows[i]["currentFlow.speed"] = data["currentFlow.subSegments"][i][
                "speed"
            ]
        except:
        ...
        try:
            new_rows[i]["currentFlow.speedUncapped"] = data["currentFlow.subSegments"][
                i
            ]["speedUncapped"]
        except:
        ...
        new_rows[i]["currentFlow.freeFlow"] = data["currentFlow.subSegments"][i][
            "freeFlow"
        ]
        new_rows[i]["currentFlow.jamFactor"] = data["currentFlow.subSegments"][i][
            "jamFactor"
        ]
    ...
    for i in range(n):
        ...
        df_list[-1]["geometry.multi_line"] = df_list[-1]["geometry.multi_line"].apply(
            lambda x: MultiLineString(x)
        )
...
def sub_segment_extraction(gdf):
    ...
    gdf_processed = pd.concat(
        [gdf_processed] + new_rows_, ignore_index=True
...
def update_data_catalog_gdf(date, trigger):
    ...
    if trigger:
        ...
        with open(catalog_path, "r+") as f:
            contents = f.read()
            result = re.search(rf"gdf{date['day']}:", contents)
            if result is None:
                contents = "\n".join(
                    [
                        contents,
                        "\n    ".join(
                            [
                                f"convert_to_gdf.{date['day']}.gdf.{date['day']}:",
        ...
        with open(catalog_path, "r+") as f:
            contents = f.read()
            result = re.search(rf"gdf{date['day']}_processed:", contents)
            if result is None:
                contents = "\n".join(
                    [
                        contents,
                        "\n    ".join(
                            [
                                f"convert_to_gdf.{date['day']}.gdf_processed.{date['day']}:",
                                f"type: pickle.PickleDataSet",
