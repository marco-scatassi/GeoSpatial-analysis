...
from retrieve_global_parameters import retrieve_gdf_average_path, retrieve_gdf_path, retrieve_catalog_path
...
def create_average_gdfs(days):
    ...
    gdfs_average = {"all_day": gdf_average, 
    ...
    if trigger:
        ...
        with open(catalog_path, "r+") as f:
            ...
            for key in ["all_day", "morning", "midday", "afternoon"]:
                result = re.search(fr"gdf_average_{key}:", contents)
                gdf_path = retrieve_gdf_average_path(key)
                if result is None:
                    contents = "\n".join([contents, 
                                    "\n    ".join([f"create_average_gdfs.{key}.gdf_average_{key}:",
