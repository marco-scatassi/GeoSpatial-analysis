...
def compose_url_to_raw_data(day: str):
    ...
    file_urls = [
        os.path.join(dir_url, file)
        for dir_url in dirs_urls
        for file in os.listdir(dir_url)
    ]
    return file_urls
...
def splitting_and_time_processing(raw_data: dict):
    ...
    for key, value in raw_data.items():
        ...
        for result in value["results"]:
            ...
            time_processed_collections_documents[-1]["api_call_time"] = api_call_times[
                key
            ]
...
def geometry_processing(input_data: list):
    ...
    for doc in input_data:
        ...
        geo_processed_data[-1]["geometry"] = GeometryCollection(
            [
                LineString([(e["lng"], e["lat"]) for e in i["points"]])
                for i in raw_data_links
            ]
        )
        # create the geometry_length field
        geo_processed_data[-1]["geometry_length"] = [
            i["length"] for i in raw_data_links
        ]
...
def process_raw_data(urls: list, day: str):
    ...
    processed_collections_documents = geometry_processing(
        time_processed_collections_documents
    )
...
def update_data_catalog_trigger(trigger, day):
    ...
    if trigger:
        ...
        with open(catalog_path, "r+") as f:
            contents = f.read()
            result = re.search(rf"cleaning.{day}.trigger_cleaning_{day}:", contents)
            if result is None:
                contents = "\n".join(
                    [
                        contents,
                        "\n    ".join(
                            [
                                f"cleaning.{day}.trigger_cleaning_{day}:",
