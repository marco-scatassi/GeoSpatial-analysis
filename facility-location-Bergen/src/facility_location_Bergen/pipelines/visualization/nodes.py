...
def get_dates(date: str):
    if date["time"] == "morning":
        ...
        last_date = datetime.strptime(
            f"{date['day']}T09:30:00.000+02:00", "%d_%m_%YT%H:%M:%S.%f%z"
        )
    elif date["time"] == "midday":
        first_date = datetime.strptime(
            f"{date['day']}T11:00:00.000+02:00", "%d_%m_%YT%H:%M:%S.%f%z"
        )
        last_date = datetime.strptime(
            f"{date['day']}T12:00:00.000+02:00", "%d_%m_%YT%H:%M:%S.%f%z"
        )
    elif date["time"] == "afternoon":
        first_date = datetime.strptime(
            f"{date['day']}T15:00:00.000+02:00", "%d_%m_%YT%H:%M:%S.%f%z"
        )
        last_date = datetime.strptime(
...
def process_gdf_to_plot_data(date: dict, gdf: gpd.GeoDataFrame):
    ...
    for i, date in enumerate(gdf_sample["api_call_time"].unique()):
        ...
        for feature, date, jam in zip(
            df["geometry.multi_line"], df["api_call_time"], df["currentFlow.jamFactor"]
        ):
            color = get_color(jam)
            ...
            df_dict[date].append(
                pd.DataFrame({"lat": lats, "lon": lons, "date": dates, "color": colors})
            )
            df_dict[date][-1] = pd.concat(
                [
                    df_dict[date][-1],
                    pd.DataFrame(
                        {
        ...
        if i % 2 == 0:
            print_INFO_message(
                f"Time to process {i+1} of {len(gdf_sample['api_call_time'].unique())} dates: {time()-t:.2f} seconds"
            )
...
def set_figure_background():
    ...
    extent = [
        center_pt[1] - (zoom * 1.3),
        center_pt[1] + (zoom * 1.6),
        center_pt[0] - zoom,
        center_pt[0] + zoom,
    ]  # adjust to zoom
    ...
    scale = np.ceil(
        -np.sqrt(2) * np.log(np.divide(zoom, 350.0))
...
def animate(i, ax, df_dict2):
    if i == 100:
        ...
        ax.set_title(
            f"Traffic across time in Bergen: {list(df_dict2.keys())[i].astimezone(tz).strftime('%Y-%m-%d %H:%M')}"
        )
        print_INFO_message(
            f"Time to plot the {i+1}th of {len(df_dict2.keys())} dates: {time()-ts:.2f} seconds"
        )
...
def update_data_catalog_gif(date, trigger):
    ...
    if trigger:
        ...
        with open(catalog_path, "r+") as f:
            contents = f.read()
            result = re.search(
                rf"visualization.gif{date['day']}{date['time']}:", contents
            )
            if result is None:
                contents = "\n".join(
                    [
                        ...
                        "\n    ".join(
                            [
                                f"visualization.gif{date['day']}{date['time']}:",
