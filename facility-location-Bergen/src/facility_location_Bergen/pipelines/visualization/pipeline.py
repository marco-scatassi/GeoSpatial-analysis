...
def create_pipeline(**kwargs) -> Pipeline:
    ...
    for e in visualization_pipeline.all_inputs():
        if "morning.gdf" in e:
            mapping[e] = e.replace("morning.gdf", "gdf").replace(
                "visualization", "convert_to_gdf"
            )
        if "midday.gdf" in e:
            mapping[e] = e.replace("midday.gdf", "gdf").replace(
                "visualization", "convert_to_gdf"
            )
        if "afternoon.gdf" in e:
            mapping[e] = e.replace("afternoon.gdf", "gdf").replace(
                "visualization", "convert_to_gdf"
            )
